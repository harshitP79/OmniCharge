...
/**
 * AOP aspect that automatically logs all RabbitMQ message publishing and
 * consumption.
 * Intercepts:
 * - @RabbitListener annotated methods (message consumption)
 * - RabbitTemplate.convertAndSend calls (message publishing)
 * 
 * This provides comprehensive SAGA event logging without modifying business
 * code.
 */
@Aspect
@Component
    ...
    public Object logPublishing(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            try {
                ...
                status = "FAILED";
                errorMessage = e.getMessage();
                log.error("RabbitMQ: Failed to publish message to exchange: {}, routingKey: {}", exchange, routingKey,
                        e);
                throw e;
            } finally {
                long duration = System.currentTimeMillis() - startTime;
